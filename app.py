...
if choice == "Custom Text Summarization":
    st.sidebar.markdown(
        "Copy Sample Article if you want to test the web app. [[article source](https://edition.cnn.com/2022/02/14/us/new-mexico-albuquerque-stabbings/index.html)]")
    ...
    with col1:
        text = st.text_area(label="Enter your text or story", height=350,
                            placeholder="Enter Your Text or story or your article it can be of any length")
...
if choice == "News Summary and Headlines":
    ...
    search_query = st.text_input(
        "", placeholder="Enter the topic you want to search")
    st.write(" ")
    ...
    if link != []:
        ...
        with col1:
            for i in range(len(link)):
                if (i % 2) == 0:
                    ...
                    st.markdown(
                        "[**Read Full Article**]({})".format(link[i]), unsafe_allow_html=True)
                    st.write(" ")
        ...
        st.info(
            "No Result found for {} Please try some popular Keywords".format(search_query))
